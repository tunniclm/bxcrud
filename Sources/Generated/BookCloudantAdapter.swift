...
public class BookCloudantAdapter: BookAdapter {
    let client: CouchDBClient
    var database: Database {
        // TODO: Properly ensure database exists
        client.createDB("book") { _, _ in }
        return client.database("book")
    }
    ...
    public func findAll(onCompletion: @escaping ([Book], Swift.Error?) -> Void) {
        database.retrieveAll(includeDocuments: true) { maybeDocuments, error in
            ...
            do {
                let models = try documents.map { _, document -> Book in
                    ...
                    let model = try Book(json: modelJSON)
    ...
    public func create(_ model: Book, onCompletion: @escaping (Book?, Swift.Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Book?, Swift.Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Book, onCompletion: @escaping (Book?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Book?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
